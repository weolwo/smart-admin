...
public interface EngineContext {
    ...
    /**
     * 「当前正在执行的脚本名」在<b>内部数据通道</b>里的 key，值是 {@code ExecutableScript.name()}。
     *
     * <p>由底层 {@code ScriptEvaluator} 在每次执行前写入，Java 函数按需读取
     * （{@code tool_log} 就靠它把日志归到具体脚本上）。
     *
     * <p>放内部通道而不是脚本变量：脚本不该读到、更不该改掉自己的身份标识 ——
     * 一旦能改，日志里的脚本名就不可信，排查时会指向错误的文件。
     *
     * <p>双下划线前缀是给人看的：dump 内部通道时一眼知道这不是业务数据。
     */
    String INTERNAL_SCRIPT_NAME = "__scriptName";
