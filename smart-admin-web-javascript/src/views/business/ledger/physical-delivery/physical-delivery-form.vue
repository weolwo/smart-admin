...
<template>
  <a-modal :title="form.id ? '编辑' : '添加'" :width="100" :open="visibleFlag" @cancel="onClose" :maskClosable="false" :destroyOnClose="true">
    <a-form ref="formRef" :model="form" :rules="rules" :label-col="{ span: 5 }">
      ...
      <a-form-item label="来源发奖提案ID" name="proposalId">
        <a-input-number style="width: 100%" v-model:value="form.proposalId" placeholder="来源发奖提案ID" />
      </a-form-item>
      <a-form-item label="触发此发货的奖品配置ID (t_prize_config.id)" name="prizeConfigId">
        <a-input-number style="width: 100%" v-model:value="form.prizeConfigId" placeholder="触发此发货的奖品配置ID (t_prize_config.id)" />
      </a-form-item>
      ...
      <a-form-item label="【字典】物流公司：SF, JD, YTO" name="logisticsCompany">
        <a-input style="width: 100%" v-model:value="form.logisticsCompany" placeholder="【字典】物流公司：SF, JD, YTO" />
      </a-form-item>
      <a-form-item label="物流单号" name="logisticsNo">
        <a-input style="width: 100%" v-model:value="form.logisticsNo" placeholder="物流单号" />
      </a-form-item>
      <a-form-item label="【字典】状态：0-待发货, 1-已发货, 2-已签收, 3-异常退回" name="status">
        <a-input-number style="width: 100%" v-model:value="form.status" placeholder="【字典】状态：0-待发货, 1-已发货, 2-已签收, 3-异常退回" />
      </a-form-item>
    </a-form>
...
<script setup>
  ...
  const formDefault = {
    ...
    prizeConfigId: undefined, //触发此发货的奖品配置ID (t_prize_config.id)
    receiverName: undefined, //收件人姓名
    receiverPhone: undefined, //收件人电话
    receiverAddress: undefined, //收件详细地址
    logisticsCompany: undefined, //【字典】物流公司：SF, JD, YTO
  ...
  const rules = {
    ...
    proposalId: [{ required: true, message: '来源发奖提案ID 必填' }],
    prizeConfigId: [{ required: true, message: '触发此发货的奖品配置ID (t_prize_config.id) 必填' }],
    ...
    logisticsCompany: [{ required: true, message: '【字典】物流公司：SF, JD, YTO 必填' }],
    logisticsNo: [{ required: true, message: '物流单号 必填' }],
  };
