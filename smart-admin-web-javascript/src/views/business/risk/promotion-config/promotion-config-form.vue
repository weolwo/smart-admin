...
<template>
  ...
  >
    <a-form ref="formRef" :model="form" :rules="rules" :label-col="{ span: 5 }" >
        ...
        <a-form-item label="触发审核的阈值"  name="reviewThreshold">
          <a-input-number style="width: 100%" v-model:value="form.reviewThreshold" placeholder="触发审核的阈值" />
        </a-form-item>
...
<script setup lang="ts">
  ...
  const formDefault = {
      ...
      reviewThreshold: undefined, //触发审核的阈值
  ...
  const rules = {
      ...
      mutexRule: [{ required: true, message: '互斥规则：存互斥的优惠配置ID数组 必填' }],
      reviewType: [{ required: true, message: '【字典】人工审核：0-免审, 1-笔笔审, 2-超阈值审 必填' }],
      reviewThreshold: [{ required: true, message: '触发审核的阈值 必填' }],
  };
